...
type Job struct {
	...
	DesiredFields     map[string]json.RawMessage `json:"desired_fields"`
	DesiredSettings   map[string]json.RawMessage `json:"desired_settings"`
	SubmittedSettings map[string]json.RawMessage `json:"submitted_settings"`
...
// SubmitSettings transmits one explicit mixed settings patch, without deriving
// prices, dates or untouched settings. It shares the owner's request ledger.
func (c *Client) SubmitSettings(ctx context.Context, requestID, expectedRevision string, values map[string]string) (Job, error) {
	if !requestPattern.MatchString(requestID) || !revisionPattern.MatchString(expectedRevision) || len(values) == 0 {
		return Job{}, &Error{Code: "invalid_request"}
	}
	intent := make(map[string]string, len(values))
	for key, value := range values {
		switch key {
		case "yuan_price", "dollar_price":
			if !pricePattern.MatchString(value) {
				return Job{}, &Error{Code: "invalid_setting_value"}
			}
		...
		case "profit_margin_percent", "air_express_price_per_kg":
			if !decimalPattern.MatchString(value) {
				return Job{}, &Error{Code: "invalid_setting_value"}
			}
			number, ok := new(big.Rat).SetString(value)
			if !ok || number.Sign() < 0 || (key == "air_express_price_per_kg" && number.Sign() == 0) || (key == "profit_margin_percent" && number.Cmp(big.NewRat(1000, 1)) > 0) {
				return Job{}, &Error{Code: "invalid_setting_value"}
			}
		...
		default:
			return Job{}, &Error{Code: "invalid_setting_field"}
		}
		intent[key] = value
	}
	body := map[string]any{"settings": intent, "request_id": requestID, "expected_state_revision": expectedRevision}
	var job Job
	err := c.call(ctx, "POST", "pricing/settings", body, requestID, expectedRevision, &job)
	if err == nil {
