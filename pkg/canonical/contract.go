...
// SameIdentity compares the source scope and revision, excluding optional
// provider metadata. Independently decoded extension maps cannot change identity.
// Callers admitting mutations must separately require a usable revision.
func (source Source) SameIdentity(other Source) bool {
	return source.ID == other.ID && source.Dataset == other.Dataset && source.Revision == other.Revision
}
