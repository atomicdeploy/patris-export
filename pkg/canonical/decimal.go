...
// LandedPrice evaluates the workbook formula with exact decimal rationals
// and rounds once, half-up, to the final whole IRT amount.
func LandedPrice(weightGrams, shippingCNYPerKg, foreignCNY, markupPercent, irtPerCNY string) (int64, error) {
	values := make([]*big.Rat, 0, 5)
	for _, input := range []string{weightGrams, shippingCNYPerKg, foreignCNY, markupPercent, irtPerCNY} {
	...
	landed := new(big.Rat).Add(foreign, shippingCost)
